...
export function ReportsClient(){
 ...
 return <><div className="toolbar no-print"><select className="field" value={type} onChange={e=>setType(e.target.value as ReportType)}><option value="daily-staff">Daily staff</option><option value="weekly-staff">Weekly staff</option><option value="students">Client attendance</option><option value="site">Daily site</option><option value="visitors">Visitor book</option></select><label>From <input autoComplete="off" className="field" type="date" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>To <input autoComplete="off" className="field" type="date" value={to} onChange={e=>setTo(e.target.value)}/></label><a className="btn primary" href={`/api/reports?type=${type}&from=${from}&to=${to}&format=csv`}><Download size={17}/> Export CSV</a><button className="btn secondary" onClick={()=>window.print()}><Printer size={17}/> Print</button></div>{error&&<div className="alert alert-error">{error}</div>}<section className="card table-wrap">{loading?<div className="empty">Loading report...</div>:data.length?<table className="table"><thead><tr>{columns.map(c=><th key={c}>{clientTerminology(c.replace(/([A-Z])/g," $1"))}</th>)}</tr></thead><tbody>{data.map((row,i)=><tr key={i}>{columns.map(c=><td key={c}>{displayReportValue(row[c])}</td>)}</tr>)}</tbody></table>:<div className="empty">No records match this report.</div>}</section></>
}
