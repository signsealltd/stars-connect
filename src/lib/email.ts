...
export async function buildDailySummary(date = localDateKey()) {
  ...
  const text = [
    ...
    ...staffRows, "", `Clients present: ${summary.studentsPresent}`, `Clients absent: ${summary.studentsAbsent}`,
    `Clients late: ${summary.studentsLate}`, `Clients unconfirmed: ${summary.studentsUnconfirmed}`,
