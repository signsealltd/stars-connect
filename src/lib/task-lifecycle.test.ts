...
describe("task lifecycle",()=>{
 ...
 it("requires a separate verifier",()=>{expect(nextTaskStatus("compliance","complete",{status:"OPEN",verificationRequired:true,attachmentCount:1},"a")).toBe("AWAITING_VERIFICATION");expect(()=>nextTaskStatus("compliance","complete",{status:"AWAITING_VERIFICATION",verificationRequired:true,attachmentCount:1,completedById:"a"},"a")).toThrow(/different/);expect(nextTaskStatus("compliance","complete",{status:"AWAITING_VERIFICATION",verificationRequired:true,attachmentCount:1,completedById:"a"},"b")).toBe("COMPLETED")});
