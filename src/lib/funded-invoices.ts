...
export async function generateFundedInvoices(id:string,actorId:string) {
  ...
  const result=await prisma.$transaction(async tx=>{
    ...
    for(const group of groups.values()) {
      ...
      const days=included.filter(c=>c.chargeRuleId).reduce((sum,c)=>sum+Number(c.quantity),0);
