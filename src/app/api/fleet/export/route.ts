...
export async function GET(req:NextRequest){return withVehicle(req,async user=>{
 ...
 const lines=[`Revision ${row.revision}${row.supersedesId?" — amendment of "+row.supersedesId:""}`,`${s.vehicle.name} - ${s.vehicle.registration}`,`Staff: ${row.staffName}`,`Mileage: ${row.mileage} | Outcome: ${row.outcome}`,`Device start: ${row.clientStartedAt.toISOString()}`,`Server submission: ${row.submittedAt.toISOString()}`,`Record: ${row.id}`,`Checklist: ${row.checklistVersion} | Declaration: ${row.declarationVersion}`,"",...s.answers.flatMap(a=>[`${a.label}: ${a.response}${a.severity?` (${a.severity})`:""}`,...(a.notes?[a.notes]:[]),...(a.response==="DEFECT"?a.imageIds.map(i=>`Private evidence reference: ${i}`):[])]),"",s.declaration,"Evidence photographs are available in the authenticated check detail view."];
 ...
 const rows=await prisma.vehicleCheck.findMany({where:{supersededBy:{is:null},checkDate:{gte:new Date(from),lte:new Date(to)}},orderBy:{submittedAt:"asc"},take:10001});if(rows.length>10000)return jsonError("Choose a smaller range (maximum 10,000 checks).",422);
