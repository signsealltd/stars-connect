...
type Entry = {
  ...
  adjustmentMinutes: number; originalMinutes: number; transportMinutes: number; preRoundedMinutes: number; roundingMinutes: number; totalPayableMinutes: number; exceptionCount: number; exceptionStatus: string;
...
export function SimpleFinanceRunReview({ mode, id }: { mode: "payroll" | "billing"; id: string }) {
  ...
  async function addPayItem(event: React.FormEvent) {
    ...
    const reasonLabels: Record<string, string> = { APPROVED_ANNUAL_LEAVE: "Approved annual leave", REPORTED_SICKNESS: "Reported sickness", AUTHORISED_OVERTIME: "Authorised overtime", MANDATORY_TRAINING: "Mandatory training", UNPAID_LEAVE: "Unpaid leave", TRANSPORT_DUTY: "Student transport duty", OTHER_AUTHORISED: "Other authorised adjustment" };
    setWorking(true); setError(""); setSuccess("");
    try {
      await action("add-adjustment", { staffId: payItemEntry.staffId, date: payItem.date, category: payItem.category, minutes: Math.round(itemHours * 60), paid: payItem.category !== "UNPAID", reason: reasonLabels[payItem.reason] }, false);
  ...
  return <>
    ...
    {payItemEntry && <div className="modal-backdrop"><form className="modal" onSubmit={addPayItem}>
      ...
      <div className="form-grid">
        <label className="form-label">Pay item type<select className="field" value={payItem.category} onChange={event => setPayItem({...payItem, category:event.target.value})}><option value="HOLIDAY">Holiday</option><option value="SICKNESS">Sickness</option><option value="OVERTIME">Overtime</option><option value="TRAINING">Training</option><option value="UNPAID">Unpaid leave</option><option value="OTHER">Other</option></select></label>
        <label className="form-label">Reason<select className="field" value={payItem.reason} onChange={event => setPayItem({...payItem, reason:event.target.value})}><option value="APPROVED_ANNUAL_LEAVE">Approved annual leave</option><option value="REPORTED_SICKNESS">Reported sickness</option><option value="AUTHORISED_OVERTIME">Authorised overtime</option><option value="MANDATORY_TRAINING">Mandatory training</option><option value="UNPAID_LEAVE">Unpaid leave</option><option value="TRANSPORT_DUTY">Student transport duty</option><option value="OTHER_AUTHORISED">Other authorised adjustment</option></select></label>
