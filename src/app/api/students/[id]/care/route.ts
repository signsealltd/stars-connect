...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){return withCapability(req,CAPABILITIES.STUDENT_CARE_EDIT,async user=>{
 ...
 const student=await prisma.student.findUnique({where:{id}});if(!student)return jsonError("Client not found",404);
 ...
 const content=[`Client: ${student.displayName}`,"Prepared from supplied information. Missing details must be confirmed before publication.",...sections.map(([key,label])=>`${label}\n${info[key]||"NOT PROVIDED — confirm with the information provider"}`)].join("\n\n");
