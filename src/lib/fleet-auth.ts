...
export async function withVehicle(req:NextRequest,handler:(user:User)=>Promise<NextResponse>){
 ...
 const session=await getSession(true);if(!session)return jsonError("Sign in to continue. Your local draft is safe.",401);
...
export async function canReviewFleet(){const s=await getSession();return !!s&&hasCapability(s.user.role,CAPABILITIES.FLEET_VIEW,s.user.permissionOverrides);}
