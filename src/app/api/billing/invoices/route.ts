...
export async function GET(req:NextRequest){return withCapability(req,CAPABILITIES.BILLING_REVIEW,async()=>{
 ...
 const [rows,total]=await Promise.all([prisma.invoice.findMany({where,include:{billingRun:{select:{label:true,periodStart:true,periodEnd:true,billingPeriodId:true}}},orderBy:[{billingRun:{periodStart:"desc"}},{payerName:"asc"},{createdAt:"desc"}],skip:(page-1)*25,take:25}),prisma.invoice.count({where})]);
