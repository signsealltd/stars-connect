...
vi.mock("./prisma",()=>({prisma:{session:{findFirst:async()=>({id:"session",scope:state.scope,lastSeenAt:new Date(),user:{id:"user",organisationId:"org",active:true}}),deleteMany:state.deleted},staffMember:{findUnique:async()=>({active:true})}}}));
