...
export async function GET(req:NextRequest){return withCapability(req,C.STAFF_TASKS,async user=>{
 ...
 if(can(C.PREMISES_VIEW)){
  ...
  const history=actions.length?await prisma.auditLog.findMany({where:{entityType:"PremisesCorrectiveAction",entityId:{in:actions.map(a=>a.id)}},select:{id:true,entityId:true,action:true,createdAt:true,afterValue:true},orderBy:{createdAt:"asc"}}):[];
  rows.push(...actions.map(t=>({id:t.id,source:"premises" as const,canChange:canChangeTask(user,"premises"),title:t.title,category:"Compliance",status:t.status,priority:t.priority,owner:t.assignedTo||"Unassigned",dueDate:t.dueDate?.toISOString(),submittedAt:t.createdAt.toISOString(),description:t.notes,notes:t.notes,related:t.asset?.name,href:"/dashboard/premises?section=actions",evidence:t.asset?.documents.map(d=>({label:d.title+(d.reference?` · ${d.reference}`:""),href:d.documentUrl||undefined})),events:history.filter(e=>e.entityId===t.id).map(e=>({id:e.id,action:e.action,createdAt:e.createdAt.toISOString(),message:String((e.afterValue as Record<string,unknown>)?.reason||"")}))})));
 }
