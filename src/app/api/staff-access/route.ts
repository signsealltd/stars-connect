...
export async function POST(req:NextRequest){return withCapability(req,CAPABILITIES.STAFF_ACCESS_MANAGE,async user=>{
 ...
 await prisma.$transaction(async tx=>{
  await tx.staffPortalAccount.upsert({where:{staffId:staff.id},create:{staffId:staff.id,loginLink:personalLink},update:{loginLink:personalLink}});
  ...
  }else if(v.action==="enable")await tx.staffPortalAccount.update({where:{staffId:staff.id},data:{enabled:true,pinHash:null,invitationHash:null,invitationExpiresAt:null,failures:0,lockedUntil:null}});
 ...
 return staffJson({ok:true,personalUrl:new URL("/staff/?member="+personalLink,process.env.APP_URL||req.nextUrl.origin).href,...(["invite","reset"].includes(v.action)?{activationUrl:new URL("/staff/activate",process.env.APP_URL||req.nextUrl.origin).href+"#"+token}: {})});
})}
