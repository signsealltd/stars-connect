...
const formatBillingDate = (value: string) => new Intl.DateTimeFormat("en-GB", {
  ...
  return <>
    ...
    <section className="card table-wrap"><table className="table">
      ...
      <tbody>{visible.map(record => mode === "payroll" ? (() => {
        ...
        return <tr key={charge.id}><td>{invoice ? <a href={`/dashboard/billing/invoices/${invoice.id}`} title={`View invoice ${invoice.invoiceNumber}`}>{charge.studentName}</a> : charge.studentName}</td><td>{formatBillingDate(charge.sourceDate)}</td><td>{missing ? "Billing details required" : <>{charge.description}{charge.manuallyAdjusted && <small className="muted" style={{display:"block"}}>Adjusted with reason recorded</small>}</>}</td><td>{money(charge.netAmount)}</td><td>{money(charge.vatAmount)}</td><td><b>{money(charge.grossAmount)}</b></td><td>{charge.excluded ? "EXCLUDED" : missing ? "SETUP REQUIRED" : charge.exceptionCode || (charge.manuallyAdjusted ? "ADJUSTED" : "CLEAR")}</td><td><div className="table-actions">{missing ? <a className="btn primary" href={`/dashboard/billing/profiles?studentId=${charge.studentId}&returnTo=${encodeURIComponent(`/dashboard/billing/runs/${id}`)}`}>Set up billing</a> : <button className="btn primary" disabled={working || complete || charge.excluded} onClick={() => openBillingAdjustment(charge)}>{charge.exceptionCode === "HISTORICAL_ATTENDANCE_REQUIRED" ? "Enter attendance days" : "Add service"}</button>}<button className="btn secondary" disabled={working || complete} onClick={() => exclude(charge, charge.excluded)}>{charge.excluded ? "Restore" : "Exclude"}</button></div></td></tr>;
      })())}</tbody>
