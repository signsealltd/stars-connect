...
export async function GET(req:NextRequest){return withCapability(req,CAPABILITIES.SAFEGUARDING_MANAGE,async()=>{
 ...
 return NextResponse.json({policy,openCount,total,page,rows:rows.map(r=>({...r,studentName:students.find(s=>s.id===r.studentId)?.displayName||"Client"}))},{headers:{"Cache-Control":"private, no-store"}});
});}
