...
{key:CAPABILITIES.STUDENT_CARE_EDIT,label:"Manage student care information",description:"Update photographs, returned care information and generate RAMS drafts."},
{key:CAPABILITIES.SYSTEM_VIEW,label:"View system and backups",description:"Inspect system health and available backups."},{key:CAPABILITIES.SYSTEM_MANAGE,label:"Manage system and backups",description:"Create, download or remove backups and run system maintenance."},{key:CAPABILITIES.PHOTO_VIEW,label:"View attendance photographs",description:"Access protected attendance photographs."},
 ...Object.entries(CAPABILITIES).filter(([key])=>["DASHBOARD", "STAFF", "STUDENTS", "REGISTER", "LIVE", "TIMESHEETS", "REPORTS", "EMERGENCY", "VISITORS", "TRAINING", "PREMISES", "DEVICES", "USERS", "SETTINGS", "AUDIT", "SYNCHRONISATION", "CONFLICTS"].some(module=>key===module+"_VIEW"||key===module+"_MANAGE")).map(([,key])=>({key,label:key.replace("."," — "),description:`Allow ${key.split(".")[1]} access to ${key.split(".")[0]}.`})),
  ...(["STAFF_PORTAL","STAFF_RESOURCES_MANAGE","STAFF_POLICIES","STAFF_RIDDOR","STAFF_SDS","STAFF_RAMS","STAFF_CALENDAR","STAFF_TRAINING","SAFEGUARDING_MANAGE","STUDENT_CARE"] as const).map(key=>({key:CAPABILITIES[key],label:key.toLowerCase().replaceAll("_"," "),description:"Allow access to this staff or care workflow."})),
