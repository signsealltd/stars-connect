...
export async function GET(req:NextRequest){return withVehicle(req,async user=>{
 const manager=await canReviewFleet(req),today=new Date(new Intl.DateTimeFormat("sv-SE",{timeZone:"Europe/London"}).format(new Date()));
 const rows=await prisma.fleetVehicle.findMany({where:{...(manager&&req.nextUrl.searchParams.get("archived")==="1"?{OR:[{deletedAt:{not:null}},{status:"ARCHIVED"}]}:{deletedAt:null,status:{not:"ARCHIVED"}})},include:{checks:{where:{checkDate:today,supersededBy:{is:null}},select:{id:true,outcome:true,submittedAt:true},orderBy:{submittedAt:"desc"}},_count:{select:{defects:{where:{status:{not:"CLOSED"}}}}}},orderBy:{name:"asc"}});
...
export async function POST(req:NextRequest){return withCapability(req,CAPABILITIES.FLEET_MANAGE,async user=>{
 ...
 try{const row=await prisma.$transaction(async tx=>{const before=id?await tx.fleetVehicle.update({where:{id,deletedAt:null},data:{updatedAt:new Date()}}):null;
...
export async function PATCH(req:NextRequest){const body=await req.json().catch(()=>null);if(!z.string().uuid().safeParse(body?.id).success||typeof body?.notes!=="string"||body.notes.trim().length<3||body.notes.length>2000)return jsonError("Enter a reason.",422);
 ...
 await prisma.$transaction(async tx=>{const before=await tx.fleetVehicle.update({where:{id:body.id,...(body.action==="reactivate"?{}:{deletedAt:null})},data:{updatedAt:new Date()}});if(body.action==="reactivate"&&before.status!=="ARCHIVED")throw Error("Vehicle is not archived.");const status=body.action==="archive"?"ARCHIVED":"OUT_OF_SERVICE";await tx.fleetVehicle.update({where:{id:body.id},data:{status,...(body.action==="reactivate"?{deletedAt:null}:{}),updatedById:user.id}});await tx.auditLog.create({data:{action:"FLEET_STATUS_CHANGED",actorType:"USER",actorId:user.id,entityType:"FleetVehicle",entityId:body.id,beforeValue:{status:before.status},afterValue:{status,notes:body.notes}}});});return NextResponse.json({ok:true});
 }catch(e){return jsonError(e instanceof Error?e.message:"Unable to update vehicle.",409);}});
}

export async function DELETE(req:NextRequest){return withCapability(req,CAPABILITIES.FLEET_MANAGE,async user=>{const body=await req.json().catch(()=>null);if(!z.string().uuid().safeParse(body?.id).success)return jsonError("Choose a vehicle.",422);const result=await prisma.$transaction(async tx=>{const vehicle=await tx.fleetVehicle.findUnique({where:{id:body.id}});if(!vehicle)return false;if(body.permanent===true){if(!vehicle.deletedAt&&vehicle.status!=="ARCHIVED")throw new RequestError("Archive this vehicle before permanent deletion.",409);const history=await tx.vehicleCheck.count({where:{vehicleId:vehicle.id}})+await tx.vehicleDefect.count({where:{vehicleId:vehicle.id}})+await tx.vehicleMaintenance.count({where:{vehicleId:vehicle.id}})+await tx.documentRecord.count({where:{sourceId:vehicle.id}});if(history)throw new RequestError("This vehicle has check, defect, document or maintenance history. It must remain in the archive to preserve these records.",409);await tx.fleetVehicle.delete({where:{id:vehicle.id}});await tx.auditLog.create({data:{action:"FLEET_VEHICLE_PERMANENTLY_DELETED",actorType:"USER",actorId:user.id,entityType:"FleetVehicle",entityId:vehicle.id,beforeValue:{name:vehicle.name,registration:vehicle.registration},afterValue:{historyCount:0}}});return true;}await tx.fleetVehicle.update({where:{id:vehicle.id},data:{deletedAt:new Date(),status:"ARCHIVED",updatedById:user.id}});await tx.auditLog.create({data:{action:"FLEET_VEHICLE_DELETED",actorType:"USER",actorId:user.id,entityType:"FleetVehicle",entityId:vehicle.id,beforeValue:{name:vehicle.name,registration:vehicle.registration,status:vehicle.status},afterValue:{deleted:true,historyRetained:true}}});return true;});return result?NextResponse.json({ok:true}):jsonError("Vehicle not found.",404);});}
