...
export async function POST(req:NextRequest){return withCapability(req,C.STAFF_TASKS,async user=>{
 ...
 const result=await prisma.$transaction(async tx=>{
  ...
  let status=nextTaskStatus(v.source,v.action,{status:task.status,...("verificationRequired" in task?{verificationRequired:task.verificationRequired,completedById:task.completedById,attachmentCount:task.attachments.length}:{})},user.id);
  if(v.source==="compliance"&&v.action==="reopen"&&task.status==="CANCELLED"){const previous=await tx.auditLog.findFirst({where:{entityType:"ComplianceAction",entityId:v.id,action:"TASK_ARCHIVE"},orderBy:{createdAt:"desc"}});if((previous?.beforeValue as Record<string,unknown>)?.status==="AWAITING_EVIDENCE")status="AWAITING_EVIDENCE";}
  ...
  }else if(v.source==="premises")await tx.premisesCorrectiveAction.update({where:{id:v.id},data:{status,completedAt:status==="COMPLETED"?new Date():null,updatedById:user.id}});
