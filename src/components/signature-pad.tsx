...
import type { SignatureStrokeData } from "@/lib/visitors";

export function SignaturePad({value,onChange}:{value:SignatureStrokeData;onChange:(value:SignatureStrokeData)=>void}){
 const canvas=useRef<HTMLCanvasElement>(null),active=useRef<{x:number;y:number;t:number}[]|null>(null),[error,setError]=useState("");
 ...
 function start(e:React.PointerEvent<HTMLCanvasElement>){e.currentTarget.setPointerCapture(e.pointerId);active.current=[point(e)];setError("")}
 function move(e:React.PointerEvent<HTMLCanvasElement>){if(!active.current)return;active.current.push(point(e));onChange([...value.slice(0,-(value.at(-1)===active.current?1:0)),active.current]);}
 function end(){if(!active.current)return;if(active.current.length<2)setError("Please draw a signature rather than tapping the box.");active.current=null}
