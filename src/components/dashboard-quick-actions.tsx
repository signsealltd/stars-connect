...
import { Settings2, Siren, Users, FileText, CalendarDays, Car, ClipboardList, Monitor, ShieldCheck, Clock3, Link2, type LucideIcon } from "lucide-react";
import { appConfirm } from "@/lib/app-dialog";

const actionIcons:Record<string,LucideIcon>={emergency:Siren,staff:Users,students:Users,billing:FileText,calendar:CalendarDays,fleet:Car,register:ClipboardList,live:Users,reports:FileText,visitors:Users,training:ClipboardList,premises:ShieldCheck,devices:Monitor,payroll:FileText,staffCalendar:CalendarDays,staffPortal:ShieldCheck,vehicleCheck:Car,timesheets:Clock3,informationReviews:FileText,healthSafety:ShieldCheck};
...
export function DashboardQuickActions({ emergency, onEmergencyClosed }: { emergency?: { id: string; missing: number; startedAt: string } | null; role?: string; onEmergencyClosed?: () => void }) {
  ...
  const selected=[...new Set([...preferences.quickActions,"vehicleCheck"])].flatMap(id=>available.filter(option=>option.id===id));
  ...
  return <section className="card dashboard-quick-actions" style={{padding:22}}>
    ...
    <div className="dashboard-shortcuts">{selected.map(item=>{const Icon=actionIcons[item.id]||Link2;return <Link className={`btn ${item.id==="emergency"?"danger":"secondary"}`} href={item.href} key={item.id}><Icon size={23} aria-hidden="true"/>{item.label}</Link>})}</div>
    {editing&&<div className="modal-backdrop"><div className="modal"><h2>Edit quick actions</h2><p className="muted">Choose up to twelve shortcuts for your dashboard. Vehicle check is managed under Fleet → User access.</p><div className="grid">{available.filter(item=>item.id!=="vehicleCheck").map(item=><label key={item.id}><input type="checkbox" checked={preferences.quickActions.includes(item.id)} disabled={!preferences.quickActions.includes(item.id)&&preferences.quickActions.length>=12} onChange={e=>setPreferences({...preferences,quickActions:e.target.checked?[...preferences.quickActions,item.id]:preferences.quickActions.filter(id=>id!==item.id)})}/> {item.label}</label>)}</div><div className="modal-actions"><button className="btn secondary" onClick={()=>setEditing(false)}>Cancel</button><button className="btn primary" onClick={save}>Save quick actions</button></div></div></div>}
  </section>;
}
