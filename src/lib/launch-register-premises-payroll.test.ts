...
describe("register, premises and payroll launch fixes",()=>{
  it("refreshes the complete active client cache and uses manager navigation",()=>{const source=read("src/app/register/page.tsx");expect(source).toContain('await local.clear("students")');expect(source).toContain('/api/students/records?status=active');expect(source).toContain('Return to dashboard');});
  it("provides a dedicated RAMS and COSHH area",()=>{const source=read("src/components/premises-manager.tsx");expect(readFileSync("src/components/safety-hub.tsx","utf8")).toContain("RAMS Studio");expect(source).toContain('["coshh","COSHH"]');expect(source).toContain('["RAMS","COSHH"]');});
