...
export const capabilityOptions=[
  ...
  {key:CAPABILITIES.COMPLIANCE_ACKNOWLEDGE,label:"Acknowledge documents",description:"Acknowledge assigned published compliance documents."},
  ...
  {key:CAPABILITIES.CALENDAR_MANAGE,label:"Manage operational calendar",description:"Manage permitted calendar source records."},] as const;
...
const managerReviewDefaults=new Set<Capability>([CAPABILITIES.INFORMATION_REVIEW_VIEW,CAPABILITIES.INFORMATION_REVIEW_MANAGE,CAPABILITIES.INFORMATION_REVIEW_APPROVE]);
export const hasCapability=(role:Role,capability:Capability,permissionOverrides?:unknown)=>overrides(permissionOverrides)[capability]??(grants[role].has(capability)||(role==="MANAGER"&&managerReviewDefaults.has(capability)));
