...
export async function GET(req:NextRequest){
 return withRole(req,"RECEPTION",async user=>{
  ...
  const[events,attendance,students,devices,conflicts,corrections,rollCall,email]=await Promise.all([
  ...
  return NextResponse.json({
   role:user.role,date,staffIn:staffIn.length,present:attendance.filter(a=>a.status==="PRESENT"||a.status==="LATE").length,
