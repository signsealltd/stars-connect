import {vehicleDriverSession} from "./vehicle-driver-access";
...
export async function withVehicle(req:NextRequest,handler:(user:User)=>Promise<NextResponse>){
 ...
 const driverRoute=req.nextUrl.pathname.startsWith("/api/driver/vehicle/");
 const driver=driverRoute?await vehicleDriverSession():null;
 const staffRoute=req.nextUrl.pathname.startsWith("/api/staff-area/vehicle/");
 const staff=staffRoute?await staffSession():null;
 const session=driverRoute?(driver?{user:driver,scope:"VEHICLE"}:null):staffRoute?(staff?{user:staff.user,scope:"VEHICLE"}:null):await getSession(true);if(!session)return jsonError("Sign in to continue. Your local draft is safe.",401);
...
export async function canReviewFleet(req?:NextRequest){if(req?.nextUrl.pathname.startsWith("/api/staff-area/vehicle/")||req?.nextUrl.pathname.startsWith("/api/driver/vehicle/"))return false;const s=await getSession();return !!s&&hasCapability(s.user.role,CAPABILITIES.FLEET_VIEW,s.user.permissionOverrides);}
