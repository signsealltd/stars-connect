    ...
    <section className="card table-wrap">
      {loading ? <div className="empty">Loading…</div> : items.length ? <table className="table">
        ...
        <tbody>{items.map(item => <tr key={item.id}>
          <td>{new Date(item.periodStart).toLocaleDateString("en-GB")} – {new Date(item.periodEnd).toLocaleDateString("en-GB")}</td>
          ...
          <td><div className="table-actions">
            ...
            {["DRAFT", "REQUIRES_REVIEW", "REVIEWED"].includes(item.status) && <button className="btn secondary" disabled={working} onClick={() => refresh(item)}>Refresh calculations</button>}
          </div></td>
        </tr>)}</tbody>
