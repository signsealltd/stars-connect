...
V1.8.2 uses five fixed staff grades and automatically assigns access from the job title. See [staff grade migration and access guidance](RELEASE-V1.8.2.md).

### V1.12.0 — Medication stock, Tasks and staff HR

Production revision: **V1.11.1 → V1.12.0**. Apply `npx prisma migrate deploy` before starting the new build. Migration `20260924120000_medication_stock_staff_hr` adds medication/stock audit tables, operational task history/priority/assignment, and structured staff HR/history. It does not update or delete existing care profiles, staff rows or tasks, and imposes no new required information on them.

Medication setup: People → Medication stock, or the medication link in a client profile. Link existing care-profile notes once for a selected client, then confirm the medication, stock units, usage and counted quantity. Free text is preserved, never interpreted as a clinical dose. Staff with medication permission can use `/staff/medications` in their existing Staff session. This is stock planning, not a medication administration record or clinical prescribing system.

Estimates: regular quantity ÷ (quantity per administration × daily administrations); an optional Sunday–Saturday schedule uses daily totals instead. Decimal quantities support liquids. PRN has no estimate without a manual daily usage rate. Counted stock establishes the stock anchor; unused scheduled doses rebase projected stock without pretending the stock was counted. Deliveries require counted remaining stock plus received quantity. Corrections append history. Unit changes require stock reconfirmation; usage changes rebase the previous estimate. Run-out dates and remaining stock are always estimates. Confirmation age defaults to 30 days and can be changed. Alerts default to 14 days, urgent ≤7, critical ≤3, overdue after the estimate. Invalid, paused and discontinued records have no run-out alert.

One task per medication reminder cycle is updated on estimate/priority changes. Receipt completes that cycle; closed cycles remain in history. An automatically corrected estimate can open a fresh cycle when it reaches the reminder threshold again. Confirm stock to deliberately restart a manually closed cycle. Notification event keys deduplicate repeated scheduled checks. Dashboard notifications and Tasks are filtered by permissions. Existing calendar endpoints cannot expose or modify medication tasks.

Scheduling: the existing authenticated **POST `/api/internal/reports/daily/run`** now refreshes medication reminders before report generation, using the current scheduler and `REPORT_JOB_SECRET`. No new scheduler is necessary where this documented daily job is active. An independent **POST `/api/cron/medication-reminders`**, using the same bearer secret, is available for installations without that job. Schedule one daily invocation, monitor non-2xx results, and do not put secrets into source control. Managers may also use Check reminders now. No external scheduler has been provisioned by this code change.

Permissions: `staff-hr.view`, `staff-employment.edit`, existing `staff-profile.approve`, `staff-medical.view`, `staff-medical.edit`, `medication.view`, `medication.adjust`, `medication.manage`, `medication.complete`. Administrators retain full access. Non-administrator medical access must be explicitly granted. Existing custom access levels remain unchanged: grant these new permissions deliberately in Access Levels. Stock receipt requires both stock-adjustment and completion permissions. Existing supporting-document permissions still apply to private request attachments.

Staff HR: People → Staff → HR details. Existing core job title, employment dates, pay/reference and working pattern stay in their current profile controls; structured HR extends these rather than duplicating them. Staff submit their own personal and health changes for approval; existing direct preferred-name/telephone edits remain available and audited. Protected employment fields remain manager controlled. Management occupational-health notes are excluded from own-profile responses. Supporting documents attach to the employee's request through the existing private document workflow. Before/after history and sensitive reads are audited; detailed medical history is restricted rather than copied into ordinary audit screens.

Settings password reset: linked access-level accounts now send password/account updates without attempting to override their staff grade. Role/individual-permission controls are disabled for these accounts; password policy remains unchanged.

Rollback: stop the medication job, deploy the preceding application revision and retain the additive tables/columns. Existing application queries remain compatible; this reverses application behaviour without destroying new records. Take a verified database backup before deployment. Do not drop these tables or restore an older database over subsequent production activity. A physical schema rollback requires exporting and retaining all new audit/HR/medication records first and a separately reviewed migration.
