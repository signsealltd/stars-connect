...
export async function POST(req: NextRequest) {
  ...
  const [last, lastPresence, cameraSetting] = await Promise.all([
    prisma.clockEvent.findFirst({ where: { staffId: credential.staffId }, orderBy: { deviceTimestamp: "desc" } }),
    prisma.staffPresenceEvent.findFirst({ where: { staffId: credential.staffId }, orderBy: { deviceTimestamp: "desc" } }),
  ...
  const offsite = Boolean(clockedIn && lastPresence && lastPresence.deviceTimestamp > last.deviceTimestamp && lastPresence.type === "WENT_OFFSITE");
  return NextResponse.json({
    staffId: credential.staffId,
    displayName: credential.staff.displayName,
    nextAction: clockedIn ? "CLOCK_OUT" : "CLOCK_IN",
    clockedIn,
    offsite,
    cameraMode,
    cameraRequired,
