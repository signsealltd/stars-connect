...
describe("register, premises and payroll launch fixes",()=>{
  ...
  it("provides a dedicated RAMS and COSHH area",()=>{const source=read("src/components/premises-manager.tsx");expect(source).toContain('["rams","RAMS"]');expect(source).toContain('["coshh","COSHH"]');expect(source).toContain('["RAMS","COSHH"]');});
