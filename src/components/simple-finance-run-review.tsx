  ...
  return <>
    ...
    {mode === "billing" && <section className={`card ${styles.recordFilters}`} aria-label="Billing record filters">
      ...
      <div className={styles.filterGrid}>
        <label className="form-label">Client<select className="field" value={billingStudentId} onChange={event => setBillingStudentId(event.target.value)}><option value="ALL">All clients ({billingStudents.length})</option>{billingStudents.map(student => <option key={student.studentId} value={student.studentId}>{student.studentName}</option>)}</select></label>
