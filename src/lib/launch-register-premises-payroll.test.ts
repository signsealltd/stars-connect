...
describe("register, premises and payroll launch fixes",()=>{
  ...
  it("renders transport as a bus-marked payable addition",()=>{const builder=read("src/lib/build-payroll-timesheet.ts"),pdf=read("src/lib/payroll-timesheet-pdf.ts");expect(builder).toContain('entry: start.transportDuty || finish?.transportDuty ? "Transport"');expect(builder).toContain('clockInAllowanceMinutes + transportSettings.clockOutAllowanceMinutes');expect(pdf).toContain("busIcon");});
});