...
export function BillingPeriodSetup(){
 ...
 return <details className="card" style={{padding:20,marginBottom:20}}><summary><b>Set up billing periods</b> · {rows.length} saved</summary>
  <p>LBE uses the council’s From–To dates. Other students use complete calendar months. Each saved period creates a billing task due on its final day.</p>
  <div className="toolbar"><button className="btn secondary" disabled={busy} onClick={()=>void save({preset:"LBE_2026"})}>Load supplied LBE 2026 dates</button><label>Calendar year<input className="field" type="number" min={2020} max={2100} value={year} onChange={e=>setYear(Number(e.target.value))}/></label><button className="btn secondary" disabled={busy} onClick={()=>void save({preset:"MONTHLY",year})}>Add full monthly periods</button></div>
  ...
  <form onSubmit={e=>{e.preventDefault();void save(form);}}><h3>{form.id?"Edit period":"Add a period"}</h3><div className="form-grid">
   ...
   <label className="form-label">Billing group<select className="field" value={form.cycle} onChange={e=>setForm({...form,cycle:e.target.value})}><option value="LBE">LBE / Enfield</option><option value="MONTHLY">Other students — full month</option></select></label>
