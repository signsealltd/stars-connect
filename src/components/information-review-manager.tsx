...
export function InformationReviewManager() {
  ...
  async function removeCompleted(event: React.FormEvent) {
    ...
    else setError(body.error || "The completed review could not be deleted.");
    setBusy(false);
  }
  ...
  return <>
    ...
    <section className="card table-wrap" style={{ marginTop: 18 }}>{reviews.length ? <table className="table"><thead><tr><th>Student</th><th>Status</th><th>Source</th><th>Expires</th><th>Changes</th><th>Actions</th></tr></thead><tbody>{reviews.map(review => <tr key={review.id}><td><b>{review.student.displayName}</b><small className="muted" style={{ display: "block" }}>{review.student.internalReference || "No reference"}</small></td><td><span className={`badge ${review.status === "COMPLETED" ? "badge-success" : ["SUBMITTED", "UNDER_REVIEW", "PARTIALLY_APPROVED"].includes(review.status) ? "badge-warning" : review.status === "REVOKED" || review.status === "EXPIRED" ? "badge-danger" : "badge-neutral"}`}>{review.status.replaceAll("_", " ")}</span></td><td>{review.source}</td><td>{new Date(review.expiresAt).toLocaleDateString("en-GB")}</td><td>{review._count.proposals}</td><td><div className="table-actions"><Link className="btn secondary" href={`/dashboard/information-reviews/${review.id}`}>Open</Link>{review.status === "COMPLETED" && <button className="btn danger" onClick={() => { setDeleteReview(review); setDeletePassword(""); setError(""); }}><Trash2/>Delete test</button>}</div></td></tr>)}</tbody></table> : <div className="empty"><RefreshCw/>No information reviews have been created.</div>}</section>
    {deleteReview && <div className="modal-backdrop"><form className="modal" onSubmit={removeCompleted}><h2>Delete completed review?</h2><p>This removes the submitted form, proposals and request history for <b>{deleteReview.student.displayName}</b>. It does not reverse information already accepted into the student profile. The deletion remains in the audit log.</p><label className="form-label">Enter your password to confirm<input className="field" type="password" autoComplete="current-password" required value={deletePassword} onChange={event => setDeletePassword(event.target.value)}/></label><div className="modal-actions"><button type="button" className="btn secondary" onClick={() => setDeleteReview(null)}>Cancel</button><button className="btn danger" disabled={busy || !deletePassword}>{busy ? "Deleting..." : "Delete completed test"}</button></div></form></div>}
  </>;
}
