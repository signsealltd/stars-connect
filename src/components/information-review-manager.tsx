  ...
  return <>
    <section className="card" style={{ padding: 22 }}>
      ...
      <div className="form-grid">
        <label>Client<select className="field" value={studentId} onChange={event => setStudentId(event.target.value)}><option value="">Choose a client</option>{students.map(student => <option key={student.id} value={student.id}>{student.displayName}{student.internalReference ? ` - ${student.internalReference}` : ""}</option>)}</select></label>
