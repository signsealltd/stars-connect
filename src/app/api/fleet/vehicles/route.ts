...
export async function GET(req:NextRequest){return withVehicle(req,async user=>{
 ...
 const rows=await prisma.fleetVehicle.findMany({where:{deletedAt:null,...(manager?{}:{status:{not:"ARCHIVED"}})},include:{checks:{where:{checkDate:today,supersededBy:{is:null}},select:{id:true,outcome:true,submittedAt:true},orderBy:{submittedAt:"desc"}},_count:{select:{defects:{where:{status:{not:"CLOSED"}}}}}},orderBy:{name:"asc"}});
