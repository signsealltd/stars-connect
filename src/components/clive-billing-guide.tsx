...
export function CliveBillingGuide({ topic, onClose,topics=billingHelpTopics }: { topic: BillingHelpTopic; onClose: () => void;topics?:readonly BillingHelpTopic[] }) {
  ...
  useEffect(() => {
    ...
    const area = document.getElementById(current.target)||document.querySelector<HTMLElement>(`[data-help="${current.target}"]`);
  ...
  return <aside className="clive-billing-guide no-print" aria-label="Clive billing guidance">
    ...
    <p role="status">{found ? "The relevant area is outlined on the page." : "This control is on another page or step. Follow the instructions below, then choose the topic again."}</p>
    <p>{current.content}</p>
    <label>Show another topic<select className="field" value={current.id} onChange={e => setCurrent(topics.find(t => t.id === e.target.value)!)}>{topics.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}</select></label>
  </aside>;
}
