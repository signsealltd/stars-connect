...
describe("staff access levels",()=>{
 ...
 it("shows modules granted through a custom level and hides denied ones",()=>{const labels=managerNavForRole("TEAM_LEADER",["staff-portal.view","students.view"]).flatMap(g=>g.items.map(i=>i.label));expect(labels).toContain("Students");expect(labels).not.toContain("Billing");expect(labels).not.toContain("Users & Permissions")});
});
