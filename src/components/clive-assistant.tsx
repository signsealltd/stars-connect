...
export function CliveAssistant() {
  ...
  return <>
    ...
    <button ref={launcher} hidden={open} type="button" className="clive-launcher no-print" aria-controls="clive-panel" onClick={() => setOpen((current) => !current)} aria-expanded={open} aria-label={open ? "Close Clive" : "Ask Clive for help"}><CliveImage className="clive-launcher-image" /><span>{open ? "Close" : "Ask Clive"}</span></button>
  </>;
}
